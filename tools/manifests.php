...
foreach ($files as $file) {
    ...
    if (preg_match('/(?:use|extends|implements) +Kumwe\\\\(?:App|Extension)\\\\/', $bytes)) { throw new RuntimeException('Historical owner dependency: ' . $file->getPathname()); }
    $relative = substr($file->getPathname(), strlen($root . '/src/'), -4);
    $name = $prefix . str_replace('/', '\\', $relative);
    $class = new ReflectionClass($name);
    if (str_contains($class->getDocComment() ?: '', '@internal')) { continue; }
    $members = [];
    $docs .= '## ' . $name . "\n\n" . ($class->getDocComment() ?: '') . "\n\n";
    ...
    foreach ($class->getReflectionConstants() as $constant) {
        if ($constant->isPublic()) { $constants[] = $constant->getName(); }
    }
